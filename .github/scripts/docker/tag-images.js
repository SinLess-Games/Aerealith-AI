#!/usr/bin/env node
// .github/scripts/docker/tag-images.js
// =============================================================================
// Aerealith AI — Docker Image Tagger
// -----------------------------------------------------------------------------
// Purpose:
//   Create local Docker image tags from direct inputs, Docker config, discovery
//   artifacts, or build artifacts.
//
// Input:
//   - .github/docker/tag-images.json
//   - .github/docker/tag-images.jsonc
//   - .github/docker/tag-images.yaml
//   - .github/docker/tag-images.yml
//   - .github/docker/images.json
//   - artifacts/docker/build-images.json
//   - artifacts/ci/dockerfiles.json
//
// Output:
//   - artifacts/docker/tag-images.json
//   - artifacts/docker/tag-images.md
//
// Notes:
//   - CommonJS only.
...
//   - Dry-run mode reports docker tag commands without mutating Docker.
...
try {
  ...
  logger = {
    info: (message) => console.log(`[docker-tag] ${message}`),
    warn: (message) => console.warn(`[docker-tag] WARN: ${message}`),
    error: (message) => console.error(`[docker-tag] ERROR: ${message}`),
    debug: () => {},
    dump: () => {},
    formatError: (err) => {
...
const DEFAULT_CONFIG_CANDIDATES = [
  ...
  "docker/tag-images.json",
  "docker/tag-images.jsonc",
  "docker/tag-images.yaml",
  "docker/tag-images.yml",
  "docker/images.json",
  "docker/images.jsonc",
  "docker/images.yaml",
  "docker/images.yml",
...
const DEFAULT_OUTPUT_FILE = "artifacts/docker/tag-images.json";
const DEFAULT_SUMMARY_FILE = "artifacts/docker/tag-images.md";
...
function parseArgs(argv = process.argv.slice(2)) {
  const args = {
    ...
    config_file: process.env.DOCKER_TAG_IMAGES_CONFIG_FILE || "",
    build_report_file:
      process.env.DOCKER_TAG_IMAGES_BUILD_REPORT_FILE ||
      DEFAULT_BUILD_REPORT_FILE,
    dockerfiles_file:
      process.env.DOCKER_TAG_IMAGES_DOCKERFILES_FILE ||
      DEFAULT_DOCKERFILES_FILE,

    output_file:
      process.env.DOCKER_TAG_IMAGES_OUTPUT_FILE || DEFAULT_OUTPUT_FILE,
    summary_file:
      process.env.DOCKER_TAG_IMAGES_SUMMARY_FILE || DEFAULT_SUMMARY_FILE,

    registry:
      process.env.DOCKER_TAG_IMAGES_REGISTRY ||
      process.env.CONTAINER_REGISTRY ||
      DEFAULT_REGISTRY,
    namespace:
      process.env.DOCKER_TAG_IMAGES_NAMESPACE ||
      process.env.DOCKER_IMAGE_NAMESPACE ||
      DEFAULT_IMAGE_NAMESPACE,

    source_image:
      process.env.DOCKER_TAG_SOURCE_IMAGE ||
      process.env.DOCKER_SOURCE_IMAGE ||
      "",
    target_image:
      process.env.DOCKER_TAG_TARGET_IMAGE ||
      process.env.DOCKER_TARGET_IMAGE ||
      "",
    image: process.env.DOCKER_TAG_IMAGE || process.env.DOCKER_IMAGE || "",
    image_name:
      process.env.DOCKER_TAG_IMAGE_NAME || process.env.DOCKER_IMAGE_NAME || "",

    source_refs: normalizeStringList(process.env.DOCKER_TAG_SOURCE_REFS),
    target_refs: normalizeStringList(process.env.DOCKER_TAG_TARGET_REFS),
    tags: normalizeStringList(process.env.DOCKER_TAG_IMAGES_TAGS),
    target_tags: normalizeStringList(process.env.DOCKER_TAG_IMAGES_TARGET_TAGS),

    include_images: normalizeStringList(process.env.DOCKER_TAG_IMAGES_INCLUDE),
    exclude_images: normalizeStringList(process.env.DOCKER_TAG_IMAGES_EXCLUDE),
    include_projects: normalizeStringList(
      process.env.DOCKER_TAG_IMAGES_INCLUDE_PROJECTS,
    ),
    exclude_projects: normalizeStringList(
      process.env.DOCKER_TAG_IMAGES_EXCLUDE_PROJECTS,
    ),

    use_config: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_USE_CONFIG,
      true,
    ),
    use_build_report: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_USE_BUILD_REPORT,
      true,
    ),
    use_dockerfiles: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_USE_DOCKERFILES,
      true,
    ),

    tag: normalizeBoolean(process.env.DOCKER_TAG_IMAGES_TAG, true),
    pull_before_tag: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_PULL,
      false,
    ),
    inspect_local: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_INSPECT_LOCAL,
      false,
    ),
    verify_after_tag: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_VERIFY,
      false,
    ),

    fail_if_empty: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_FAIL_IF_EMPTY,
      false,
    ),
    fail_on_error: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_FAIL_ON_ERROR,
      true,
    ),
    continue_on_error: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_CONTINUE_ON_ERROR,
      false,
    ),

    max_images: normalizeInteger(process.env.DOCKER_TAG_IMAGES_MAX_IMAGES, 0),
    max_refs: normalizeInteger(process.env.DOCKER_TAG_IMAGES_MAX_REFS, 0),
    timeout_minutes: normalizeInteger(
      process.env.DOCKER_TAG_IMAGES_TIMEOUT_MINUTES,
      20,
    ),
    max_buffer_mb: normalizeInteger(
      process.env.DOCKER_TAG_IMAGES_MAX_BUFFER_MB,
      128,
    ),

    dry_run: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_DRY_RUN ||
        process.env.DRY_RUN ||
        process.env.PROJECT_SYNC_DRY_RUN,
      false,
    ),
    write_summary_file: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_WRITE_SUMMARY,
      true,
    ),
    print: normalizeBoolean(process.env.DOCKER_TAG_IMAGES_PRINT, true),
    write_step_summary: normalizeBoolean(
      process.env.DOCKER_TAG_IMAGES_STEP_SUMMARY,
      true,
    ),
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--target-tag" || arg === "--target-tags" || arg === "-t") {
      args.target_tags.push(...normalizeStringList(argv[index + 1]));
      index += 1;
      continue;
    }

    if (arg === "--tag" || arg === "--do-tag") {
      args.tag = true;
      continue;
    }
    ...
    if (arg === "--pull") {
      args.pull_before_tag = true;
      continue;
    }

    if (arg === "--no-pull") {
      args.pull_before_tag = false;
      continue;
    }
    ...
    if (arg === "--verify") {
      args.verify_after_tag = true;
      continue;
    }

    if (arg === "--no-verify") {
      args.verify_after_tag = false;
      continue;
    }
...
Usage:
  node .github/scripts/docker/tag-images.js [options]

Examples:
  node .github/scripts/docker/tag-images.js --dry-run
  node .github/scripts/docker/tag-images.js --source-image aerealith-api:local --target-image ghcr.io/sinless-games/aerealith-api:latest
  node .github/scripts/docker/tag-images.js --build-report artifacts/docker/build-images.json --target-tag latest
  node .github/scripts/docker/tag-images.js --image-name aerealith-api --source-ref aerealith-api:sha --target-tag staging

Options:
      --repo <owner/repo>              Repository slug.
      --config <file>                  Tag config file.
      ...
      --source-tag <list>              Source tag list for generated source refs.
  -t, --target-tag <list>              Target tag list.
      --tag                            Run docker tag. Default.
      --no-tag                         Report only; do not tag.
      ...
      --use-config                     Use tag config. Default.
      --no-config                      Ignore tag config.
      ...
      --pull                           Pull source images before tagging.
      --inspect-local                  Inspect source refs locally before tagging.
      --verify                         Inspect target refs after tagging.
      --fail-if-empty                  Exit non-zero when no images are selected.
      --fail-on-error                  Exit non-zero on tag failure. Default.
      --no-fail-on-error               Do not fail when tagging fails.
      --continue-on-error              Continue after a tag failure.
      --no-continue-on-error           Stop after first failure. Default.
      --max-images <number>            Maximum image plans to tag.
...
function parseSimpleTagYaml(text) {
  const config = {};
  const images = [];
...
function readConfigFile(filePath, repoRoot) {
  ...
  if (extension === ".yaml" || extension === ".yml") {
    return parseSimpleTagYaml(text);
  }
...
function safeId(value) {
  return (
    normalizeString(value, "docker-tag")
      .toLowerCase()
      .replace(/^@/, "")
      .replace(/[^a-z0-9_.:/-]+/g, "-")
      .replace(/-+/g, "-")
      .replace(/^[-/:]+|[-/:]+$/g, "") || "docker-tag"
  );
}
...
function createTagRefs(record, args, repoRoot) {
  ...
  const recordSourceTags = normalizeStringList(
    record.tags || record.tag || args.tags,
  )
    .map(normalizeTag)
    .filter(Boolean);
  const effectiveSourceTags = recordSourceTags.length
    ? recordSourceTags
    : defaultTags(repoRoot);
  ...
  const recordTargetTags = normalizeStringList(
    record.target_tags || record.targetTags || args.target_tags,
  )
    .map(normalizeTag)
    .filter(Boolean);

  const effectiveTargetTags = recordTargetTags.length
    ? recordTargetTags
    : sourceTags.length
      ? sourceTags
      : effectiveSourceTags.length
        ? effectiveSourceTags
        : ["latest"];
  ...
  if (!cleanSourceRefs.length && cleanTargetRefs.length) {
    ...
    for (let index = 0; index < cleanTargetRefs.length; index += 1) {
      refs.push({
        ...
        tag_required: cleanSourceRefs[index] !== cleanTargetRefs[index],
      });
    }
  } else {
    ...
    for (const targetRef of cleanTargetRefs) {
      ...
      refs.push({
        ...
        tag_required: (matchedByTag || primarySourceRef) !== targetRef,
      });
    }
  }

  const validRefs = refs.filter((ref) => ref.source_ref && ref.target_ref);
  const uniqueRefs = dedupeTagRefs(validRefs);

  return args.max_refs > 0 ? uniqueRefs.slice(0, args.max_refs) : uniqueRefs;
}

function normalizeTagPlan(record, args, repoRoot, sourceType = "config") {
  const name = normalizeImageName(
    record.name ||
      record.image_name ||
      imageNameFromRef(
  ...
  const tagRefs = createTagRefs(record, args, repoRoot);

  return {
    id: safeId(
      `${sourceType}:${name}:${tagRefs.map((item) => item.target_ref).join(",")}`,
    ),
    ...
    tag: normalizeBoolean(record.tag, args.tag),
    pull_before_tag: normalizeBoolean(
      record.pull_before_tag ?? record.pull,
      args.pull_before_tag,
    ),
    inspect_local: normalizeBoolean(
    ...
    verify_after_tag: normalizeBoolean(
      record.verify_after_tag ?? record.verify,
      args.verify_after_tag,
    ),
    tag_refs: tagRefs,
  };
}

function dedupeTagRefs(refs) {
  const seen = new Map();
...
function dedupePlans(plans) {
  ...
  for (const plan of plans) {
    ...
    seen.set(key, {
      ...existing,
      ...plan,
      tag_refs: dedupeTagRefs([
        ...(existing.tag_refs || []),
        ...(plan.tag_refs || []),
      ]),
      tag: existing.tag || plan.tag,
      pull_before_tag: existing.pull_before_tag || plan.pull_before_tag,
      verify_after_tag: existing.verify_after_tag || plan.verify_after_tag,
...
function createPlans(args, repoRoot) {
  ...
  const selected = dedupePlans(
    records.map((record) =>
      normalizeTagPlan(record, args, repoRoot, record.source_type || "config"),
    ),
  )
...
function validatePlan(plan) {
  ...
  if (!plan.name) {
    errors.push("Tag plan is missing an image name.");
  }

  if (!plan.tag_refs.length) {
    errors.push("Tag plan produced no image references.");
  }

  for (const ref of plan.tag_refs) {
    if (!ref.source_ref) {
      errors.push(
    ...
    if (ref.source_ref === ref.target_ref) {
      warnings.push(`Source and target are identical: ${ref.source_ref}`);
    }
  }

  if (!plan.tag) {
    warnings.push("Docker tag execution is disabled for this plan.");
  }
...
function tagRef(ref, plan, args, repoRoot) {
  const startedAt = new Date();
  ...
  try {
    ...
    if (!plan.tag) {
      ...
      result.success = true;
      result.warnings.push("Docker tag execution is disabled.");
      return result;
    }
    ...
    if (ref.source_ref === ref.target_ref) {
      result.status = args.dry_run ? "planned" : "skipped";
      result.success = true;
      result.warnings.push("Source and target are identical.");
      return result;
    }
      ...
      if (!verifyCommand.success) {
        ...
        result.errors.push(
          verifyCommand.error ||
            ...
            `Failed to verify tagged image ${ref.target_ref}.`,
        );
        return result;
      }
    }

    result.status = args.dry_run ? "planned" : "tagged";
    result.success = true;
...
function tagImage(plan, args, repoRoot) {
  const startedAt = new Date();
  ...
  const result = {
    ...
    duration_ms: 0,
    tag: plan.tag,
    pull_before_tag: plan.pull_before_tag,
    inspect_local: plan.inspect_local,
    verify_after_tag: plan.verify_after_tag,
    tag_refs: plan.tag_refs,
    ref_results: [],
    validation,
    errors: [],
    warnings: validation.warnings,
    totals: {
      refs: plan.tag_refs.length,
      planned: 0,
      tagged: 0,
      skipped: 0,
      failed: 0,
  ...
  try {
    ...
    logger.info(
      `${args.dry_run ? "Planning" : "Tagging"} Docker image ${plan.name}.`,
    );

    for (const ref of plan.tag_refs) {
      const refResult = tagRef(ref, plan, args, repoRoot);
    ...
    if (result.totals.failed > 0) {
      result.status = "failed";
      result.errors.push(
        `${result.totals.failed} image reference(s) failed to tag.`,
      );
      return result;
    }
...
async function executePlans(plans, args, repoRoot) {
  ...
  for (const plan of plans.selected_images) {
    const result = tagImage(plan, args, repoRoot);
    results.push(result);

    if (!result.success && !args.continue_on_error) {
      stoppedEarly = true;
      logger.warn("Stopping after first Docker image tag failure.");
      break;
    }
  }
...
function createReport(args, repoRoot, plans, execution) {
      ...
      execution.results.flatMap((result) =>
        result.ref_results
          .filter(
            (refResult) =>
              refResult.success &&
              ["tagged", "planned"].includes(refResult.status),
          )
  ...
  return {
    ...
    config: {
      ...
      namespace: args.namespace,
      tag: args.tag,
      pull_before_tag: args.pull_before_tag,
      inspect_local: args.inspect_local,
      verify_after_tag: args.verify_after_tag,
...
function createMarkdownSummary(report) {
  const lines = [
    `# 🏷️ ${PROJECT_NAME} Docker Image Tags`,
    "",
    ...
    `- Tag execution: \`${report.config.tag ? "true" : "false"}\``,
    `- Pull first: \`${report.config.pull_before_tag ? "true" : "false"}\``,
    `- Inspect local: \`${report.config.inspect_local ? "true" : "false"}\``,
    `- Verify after tag: \`${report.config.verify_after_tag ? "true" : "false"}\``,
  ...
  if (!report.selected_images.length) {
    ...
    lines.push("| Image | Project | Source | Target | Refs | Tag |");
    lines.push("|---|---|---|---|---:|---:|");

    for (const image of report.selected_images) {
      const firstRef = image.tag_refs[0] || {};
      lines.push(
        `| \`${image.name}\` | \`${image.project || "none"}\` | \`${firstRef.source_ref || "none"}\` | \`${firstRef.target_ref || "none"}\` | \`${image.tag_refs.length}\` | \`${image.tag ? "true" : "false"}\` |`,
      );
    }
  }

  lines.push("");
  lines.push("## 🧩 Tag Results");
  lines.push("");

  if (!report.results.length) {
    lines.push("No Docker tag results were produced.");
  } else {
    lines.push(
      "| Status | Image | Project | Refs | Tagged | Failed | Duration |",
    );
    lines.push("|---|---|---|---:|---:|---:|---:|");

    for (const result of report.results) {
      lines.push(
        `| \`${result.status}\` | \`${result.name}\` | \`${result.project || "none"}\` | \`${result.totals.refs}\` | \`${result.totals.tagged}\` | \`${result.totals.failed}\` | \`${formatDuration(result.duration_ms)}\` |`,
      );
    }
  }

  if (report.tagged_refs.length) {
    lines.push("");
    lines.push("## 🏷️ Tagged References");
    lines.push("");

    for (const ref of report.tagged_refs.slice(0, 100)) {
      lines.push(`- \`${ref}\``);
    }

    if (report.tagged_refs.length > 100) {
      lines.push(
        `- ...and \`${report.tagged_refs.length - 100}\` more tagged reference(s).`,
      );
    }
  }
...
function writeGitHubOutputs(report) {
  setGitHubOutput("docker_tag_images_file", report.config.output_file);
  setGitHubOutput(
    "docker_tag_images_summary_file",
    report.config.summary_file || "",
  );
  setGitHubOutput("docker_tag_images_status", report.status);
  setGitHubOutput(
    "docker_tag_images_ok",
    report.totals.ok && !report.blocked ? "true" : "false",
  );
  setGitHubOutput(
    "docker_tag_images_selected",
    String(report.discovery.selected_images),
  );
  setGitHubOutput("docker_tag_images_tagged", String(report.totals.tagged));
  setGitHubOutput("docker_tag_images_planned", String(report.totals.planned));
  setGitHubOutput("docker_tag_images_skipped", String(report.totals.skipped));
  setGitHubOutput("docker_tag_images_failed", String(report.totals.failed));
  setGitHubOutput("docker_tag_images_invalid", String(report.totals.invalid));
  setGitHubOutput("docker_tag_images_refs", String(report.totals.refs));
  setGitHubOutput(
    "docker_tag_images_ref_tagged",
    String(report.totals.ref_tagged),
  );
  setGitHubOutput(
    "docker_tag_images_ref_failed",
    String(report.totals.ref_failed),
  );
  setGitHubOutput(
    "docker_tag_images_tagged_refs",
    report.tagged_refs.join(","),
  );
  setGitHubOutput(
    "docker_tag_images_tagged_refs_json",
    JSON.stringify(report.tagged_refs),
  );
  setGitHubOutput(
    "docker_tag_images_names",
    report.selected_images.map((image) => image.name).join(","),
  );
  setGitHubOutput(
    "docker_tag_images_names_json",
    JSON.stringify(report.selected_images.map((image) => image.name)),
  );
  setGitHubOutput(
    "docker_tag_images_failures_json",
...
async function main() {
  ...
  logger.info("Preparing Docker image tags.");

  const plans = createPlans(args, repoRoot);

  if (args.fail_if_empty && plans.selected_images.length === 0) {
    logger.error("No Docker images were selected for tagging.");
    process.exitCode = 1;
  }
  ...
  if (args.fail_on_error && report.blocked) {
    logger.error(`Docker image tagging blocked: ${report.block_reason}`);
    process.exitCode = 1;
    return;
  }
  ...
  if (args.fail_on_error && !report.totals.ok) {
    logger.error(
      `Docker image tagging completed with status "${report.status}". Failed=${report.totals.failed}, invalid=${report.totals.invalid}, ref_failed=${report.totals.ref_failed}.`,
    );
    process.exitCode = 1;
  }
}
