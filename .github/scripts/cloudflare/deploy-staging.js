...
// -----------------------------------------------------------------------------
// Purpose:
//   Deploy Cloudflare staging targets discovered by CI into Cloudflare Pages
//   staging branch deployments or Worker staging environments.
//
// Input:
...
//   - artifacts/cloudflare/deploy-staging.md
//   - artifacts/cloudflare/deploy-staging/logs/*.stdout.log
//   - artifacts/cloudflare/deploy-staging/logs/*.stderr.log
//
// Notes:
//   - CommonJS only.
...
//   - Worker staging deploys require an explicit Wrangler staging environment
//     by default to prevent accidental default-environment deployment.
// =============================================================================
...
const DEFAULT_OUTPUT_FILE = "artifacts/cloudflare/deploy-staging.json";
const DEFAULT_SUMMARY_FILE = "artifacts/cloudflare/deploy-staging.md";
...
function parseArgs(argv = process.argv.slice(2)) {
  const args = {
    ...
    environment:
      process.env.CLOUDFLARE_STAGING_ENVIRONMENT || DEFAULT_ENVIRONMENT,
    branch:
    ...
    include_targets: normalizeStringList(
      process.env.CLOUDFLARE_STAGING_INCLUDE_TARGETS,
    ),
    exclude_targets: normalizeStringList(
      process.env.CLOUDFLARE_STAGING_EXCLUDE_TARGETS,
    ),
    ...
    run_build: normalizeBoolean(process.env.CLOUDFLARE_STAGING_RUN_BUILD, true),
    build_command: process.env.CLOUDFLARE_STAGING_BUILD_COMMAND || "",

    changed_only: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_CHANGED_ONLY,
      true,
    ),
    deploy_pages: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_DEPLOY_PAGES,
      true,
    ),
    deploy_workers: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_DEPLOY_WORKERS,
      true,
    ),
    ...
    require_allowed_ref: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_REQUIRE_ALLOWED_REF,
      false,
    ),
    require_worker_staging_env: normalizeBoolean(
    ...
    allow_worker_default_staging: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_ALLOW_WORKER_DEFAULT,
      false,
    ),

    max_deployments: normalizeInteger(
      process.env.CLOUDFLARE_STAGING_MAX_DEPLOYMENTS,
      0,
    ),
    timeout_minutes: normalizeInteger(
      process.env.CLOUDFLARE_STAGING_TIMEOUT_MINUTES,
      25,
    ),
    max_buffer_mb: normalizeInteger(
      process.env.CLOUDFLARE_STAGING_MAX_BUFFER_MB,
      64,
    ),
    ...
    fail_on_error: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_FAIL_ON_ERROR,
      true,
    ),
    fail_if_empty: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_FAIL_IF_EMPTY,
      false,
    ),
    ...
    write_logs: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_WRITE_LOGS,
      true,
    ),
    write_summary_file: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_WRITE_SUMMARY,
      true,
    ),
    print: normalizeBoolean(process.env.CLOUDFLARE_STAGING_PRINT, true),
    write_step_summary: normalizeBoolean(
      process.env.CLOUDFLARE_STAGING_STEP_SUMMARY,
      true,
    ),
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--require-allowed-ref") {
      args.require_allowed_ref = true;
      continue;
    }

    if (arg === "--no-require-allowed-ref") {
    ...
    if (arg === "--require-worker-staging-env") {
      args.require_worker_staging_env = true;
      continue;
    }

    if (arg === "--allow-worker-default-staging") {
...
Examples:
  ...
  node .github/scripts/cloudflare/deploy-staging.js --target web --environment staging
  node .github/scripts/cloudflare/deploy-staging.js --all-targets --run-build
  node .github/scripts/cloudflare/deploy-staging.js --target api --allow-worker-default-staging
...
Options:
      ...
      --require-allowed-ref               Require branch/tag guard.
      --no-require-allowed-ref            Disable branch/tag guard. Default.
      --allowed-branch <list>             Allowed staging branches.
      --allowed-tag <list>                Allowed staging tags. Glob supported.
      --require-worker-staging-env        Require Worker staging env. Default.
      --allow-worker-default-staging      Allow Worker deploy without staging env.
      --max-deployments <number>          Maximum deployments to run.
      --timeout-minutes <number>          Per-command timeout. Default: 25.
      ...
      --no-fail-if-empty                  Allow empty deployment plan. Default.
...
function safeId(value) {
  return (
    normalizeString(value, "cloudflare-staging")
      .toLowerCase()
      .replace(/^@/, "")
      .replace(/[^a-z0-9_.:-]+/g, "-")
      .replace(/-+/g, "-")
...
function validateStagingGuard(args, github) {
  const errors = [];
  const warnings = [];
  ...
  const refAllowed = branchAllowed || tagAllowed;

  if (args.require_allowed_ref && !refAllowed) {
    errors.push(
      `Staging deployment is not allowed from ref "${refName}". ` +
    ...
    if (
      ...
      errors.push(
        `Worker target "${deployment.name}" does not declare a Wrangler "${args.environment}" environment. ` +
          "Enable --allow-worker-default-staging only when the default Wrangler environment is staging-safe.",
      );
    }
...
function createPlan(args, repoRoot, input) {
  ...
  const stagingGuard = validateStagingGuard(args, github);
...
function createMarkdownSummary(report) {
  const lines = [
    ...
    `- Git ref: \`${report.staging_guard.ref || report.github.ref || "unknown"}\``,
    ...
    "## 🛡️ Staging Guard",
    "",
    `- Guard check: \`${report.staging_guard.ok ? "passed" : "failed"}\``,
    `- Guard required: \`${report.config.require_allowed_ref ? "true" : "false"}\``,
    `- Branch allowed: \`${report.staging_guard.branch_allowed ? "true" : "false"}\``,
    `- Tag allowed: \`${report.staging_guard.tag_allowed ? "true" : "false"}\``,
    `- Is tag: \`${report.staging_guard.is_tag ? "true" : "false"}\``,
...
function writeGitHubOutputs(report) {
  setGitHubOutput("cloudflare_staging_file", report.config.output_file);
  setGitHubOutput(
    "cloudflare_staging_summary_file",
    report.config.summary_file || "",
  );
  setGitHubOutput("cloudflare_staging_log_dir", report.config.log_dir || "");
  setGitHubOutput("cloudflare_staging_status", report.status);
  setGitHubOutput(
    "cloudflare_staging_ok",
    report.totals.failed === 0 && !report.blocked ? "true" : "false",
  );
  setGitHubOutput("cloudflare_staging_environment", report.config.environment);
  setGitHubOutput("cloudflare_staging_branch", report.config.branch);
  setGitHubOutput(
    "cloudflare_staging_selected_deployments",
    String(report.totals.selected_deployments),
  );
  setGitHubOutput(
    "cloudflare_staging_pages",
  ...
  setGitHubOutput("cloudflare_staging_passed", String(report.totals.passed));
  setGitHubOutput("cloudflare_staging_failed", String(report.totals.failed));
  setGitHubOutput(
    "cloudflare_staging_blocked",
    report.blocked ? "true" : "false",
  );
  setGitHubOutput("cloudflare_staging_block_reason", report.block_reason || "");
  setGitHubOutput(
    "cloudflare_staging_guard_ok",
    report.staging_guard.ok ? "true" : "false",
  );
  setGitHubOutput("cloudflare_staging_urls", report.urls.join(","));
  setGitHubOutput("cloudflare_staging_urls_json", JSON.stringify(report.urls));
  setGitHubOutput(
...
function main() {
  ...
  if (args.fail_on_error && report.blocked) {
    logger.error(
      `Cloudflare staging deployment blocked: ${report.block_reason}`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (args.fail_on_error && report.totals.failed > 0) {
    logger.error(
      `Cloudflare staging deployment failed with ${report.totals.failed} failed command(s).`,
    );
    process.exitCode = 1;
  }
}
