...
const DEFAULT_USER_SERVICE_PATH_PREFIX = '/api/V1/users';
...
function getUserSettingsUrl(username: string): string {
  ...
  const pathPrefix =
    process.env.USER_SERVICE_BASE_PATH ?? DEFAULT_USER_SERVICE_PATH_PREFIX;

  return `${normalizedBaseUrl}${pathPrefix}/${encodeURIComponent(username)}/settings`;
}
