...
const DEFAULT_AUTH_RESEND_VERIFICATION_PATH =
...
function getAuthResendVerificationUrl(request: NextRequest): string {
  const explicitResendVerificationUrl =
    process.env.AUTH_SERVICE_RESEND_VERIFICATION_URL ??
    process.env.AUTH_RESEND_VERIFICATION_URL;

  if (explicitResendVerificationUrl) {
    return appendSearchParams(explicitResendVerificationUrl, request);
  }
  ...
  if (!authServiceUrl) {
    throw new Error(
      'Missing AUTH_SERVICE_URL, AUTH_SERVICE_INTERNAL_URL, or AUTH_SERVICE_RESEND_VERIFICATION_URL.',
    );
  }
  ...
  return appendSearchParams(
    joinUrl(
      authServiceUrl,
      process.env.AUTH_SERVICE_RESEND_VERIFICATION_PATH ??
        DEFAULT_AUTH_RESEND_VERIFICATION_PATH,
    ),
    request,
  );
}
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const authResponse = await fetch(getAuthResendVerificationUrl(request), {
      method: 'POST',
