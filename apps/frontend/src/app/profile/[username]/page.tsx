...
function createUnavailablePublicProfile(
  username: string,
): PublicUserProfileDto {
  const now = new Date().toISOString();
  const unavailableId = `unavailable:${username}` as UserServiceUserId;
...
function getPublicProfileUrl(username: string): string {
  ...
  return joinUrl(
    userServiceUrl,
    `${profilePathPrefix}/${encodeURIComponent(username)}/profile`,
  );
}
...
function hasSessionCookie(cookieStore: CookieStore): boolean {
  return Boolean(
...
function isOwnProfile(input: {
  ...
  return (
    input.hasSession &&
    input.sessionUsername?.trim().toLowerCase() ===
      input.requestedUsername.trim().toLowerCase()
  );
}
...
async function getPublicProfile(
  ...
  try {
    const request = () =>
      fetch(getPublicProfileUrl(username), {
    ...
    if (
      ...
      return createUnavailablePublicProfile(username);
    }
  ...
  if (!response.ok) {
    throw new Error(`Unable to fetch public profile for ${username}.`);
  }
  ...
  if (!body.ok || !body.data) {
    throw new Error(
      `User service returned an invalid public profile for ${username}.`,
    );
  }
...
async function getPrivateProfile(
  ...
  try {
    const request = () =>
      fetch(getPrivateProfileUrl(username), {
        headers: {
  ...
  if (!response.ok) {
    throw new Error(`Unable to fetch private profile for ${username}.`);
  }
  ...
  if (!body.ok || !body.data) {
    throw new Error(
      `User service returned an invalid private profile for ${username}.`,
    );
  }
...
export default async function UserProfilePage({
  ...
  const sessionUsername = cookieStore.get(COOKIE.USERNAME)?.value;
  const hasSession = hasSessionCookie(cookieStore);
  const usePrivateProfile = isOwnProfile({
    requestedUsername,
    sessionUsername,
    hasSession,
  });
  ...
  if (usePrivateProfile && !featureFlags['profile-private']) {
    notFound();
  }

  if (!usePrivateProfile && !featureFlags['profile-public']) {
    notFound();
  }

  const profile: ProfilePageData = usePrivateProfile
    ? {
        mode: 'private',
        data: await getPrivateProfile(requestedUsername),
      }
    : {
        mode: 'public',
        data: await getPublicProfile(requestedUsername),
      };
  ...
  return (
    <ProfileDashboard data={profile.data} flags={featureFlags} mode="public" />
  );
}
